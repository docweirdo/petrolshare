...
class LogList extends ChangeNotifier {
  ...
  bool hasBeenCalled =
      false; //has the refresh function been called at least once?
  ...
  Future<void> add(LogModel log) async {
    await _pool.collection('logs').add(log.toMap()).then((value) {
      _logs.add(log);
  ...
  Future<UnmodifiableListView<LogModel>> refreshLogs() async {
    ...
    return query.getDocuments().then((value) {
      _logs = value.documents.map((DocumentSnapshot doc) {
        _members[doc['uid']] ??=
            UserModel(doc['uid'], "Monsieur Impossible", null, false, 'ghost');
  ...
  Future<void> fetchPoolMembers() async {
    ...
    if (doc['members'] != null) {
      ...
      doc['members'].forEach((uid, properties) {
        _members[uid] = UserModel(uid, properties['name'],
            properties['photoURL'], true, properties['role']);
